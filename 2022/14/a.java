...
class Cave{
  ...
  public Cave(int x1, int x2 , int y){
    isoccupied = new boolean[y+1][x2-x1+1];
  ...
  public boolean dropsand(){
    ...
    y = 0;
    while(true){
      if((y == isoccupied.length-1) || 
         (x == 0 && isoccupied[y+1][x]) || 
         (x == isoccupied[0].length-1 && isoccupied[y+1][x] && isoccupied[y+1][x-1])){
        return false;
      }
      else if(!isoccupied[y+1][x]){
        y++;
      }
