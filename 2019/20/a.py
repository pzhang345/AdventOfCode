...
hasGone[letters["AA"][0][1]][letters["AA"][0][0]] = True
q = deque()
q.append((letters["AA"][0],0))
...
while q and not foundZZ:
    ...
    for i in possible:
        nx = x + i[1]
        ny = y + i[0]
        if letters["ZZ"][0] == (nx,ny):
            answer = count + 1
            foundZZ = True
            break
        if not hasGone[ny][nx] and maze[ny][nx] == ".":
            hasGone[ny][nx] = True
            q.append(((nx,ny),count + 1))
    if (x,y) in portal and not hasGone[portal[(x,y)][1]][portal[(x,y)][0]]:
        nx,ny = portal[(x,y)]
        hasGone[ny][nx] = True
        q.append(((nx,ny),count + 1))
